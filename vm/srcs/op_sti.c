...
//		if (((exe->arg2->type != 'i' && (short)test < 0) || (exe->arg2->type == 'i' && (intmax_t)(exe->arg2->d_data + (short)exe->arg3->d_data) < 0 )) && (exe->arg2->d_data + (short)exe->arg3->d_data) + exe->process->pc >= -65536 )
		tmp = ((short)((((test % IDX_MOD) + exe->process->pc) % IDX_MOD)));
		if (((exe->arg2->type != 'i' && (short)test < 0) || (exe->arg2->type == 'i' && (intmax_t)(exe->arg2->d_data + (short)exe->arg3->d_data) < 0 )))
		{
...
//				if (exe->arg2->d_data + exe->arg3->d_data + exe->process->pc < 0)
				{
					ft_printf("(with pc and mod %hd)\n", -512 + (tmp - (512 * ((tmp / 512 - 1)))));
					pc_adv = MEM_SIZE + tmp;
					arena->mem[pc_adv] = exe->process->reg[exe->arg1->d_value][0];
					arena->mem[pc_adv + 1] = exe->process->reg[exe->arg1->d_value][1];
					arena->mem[pc_adv + 2] = exe->process->reg[exe->arg1->d_value][2];
					arena->mem[pc_adv + 3] = exe->process->reg[exe->arg1->d_value][3];
				}
				else
				{
//					ft_printf("\ntmp is: %d\n", tmp);
...
//					ft_printf("test: %d | (short)test: %hd | tmp: %d | exe->arg3->d_data: %d\n", test, (short)test, tmp, exe->arg3->d_data);
		...
		else if (exe->arg2->d_data + (short)exe->arg3->d_data + exe->process->pc >= MEM_SIZE)
